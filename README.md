...
`peon` is a compiled Rust binary registered as a Claude Code hook for `SessionStart`, `UserPromptSubmit`, `Stop`, and `Notification` events. The architecture follows a Functional Core / Imperative Shell pattern:

- **`peon_core`** (pure library): Event routing, sound selection, annoyed detection, agent detection, tab title formatting — all deterministic, no I/O.
- **`peon`** (binary shell): CLI (clap), stdin hook handler, platform integration (audio via `afplay`, desktop notifications via AppleScript, terminal tab titles via ANSI escapes).

On each event, the binary reads JSON from stdin, routes it through the core to determine actions (play sound, set tab title, send notification), picks a random voice line (avoiding repeats), and executes platform commands.
